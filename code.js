...
function createNightlyBackup() {
  ...
  const dateStr = Utilities.formatDate(new Date(), timeZone, "yyyy-MM-dd_HH-mm");
  const backupName = `BACKUP_SistersBrows_${dateStr}`;
  
  // 1. Get (or create) the Backup Folder
  const folderName = "SistersBrows_Backups";
  let folder;
  ...
  // 2. Make a copy of the ENTIRE file
  const fileId = ss.getId();
  const file = DriveApp.getFileById(fileId);
  file.makeCopy(backupName, folder);
  
  // 3. (Optional) Delete old backups to save space (keep last 30)
