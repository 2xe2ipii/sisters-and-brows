        ...
        return (
          <div
            ...
            className={`
              ...
              ${isSelected 
                ? 'border-[#e6c200] bg-[#fffdf5] shadow-sm scale-[1.02]' 
                : 'border-slate-100 bg-white hover:border-slate-200 hover:bg-slate-50'
              }
            `}
          >
            <Icon className={`w-5 h-5 mb-1 ${isSelected ? 'text-[#202124]' : 'text-slate-400 group-hover:text-slate-500'}`} />
            
            <span className={`text-sm font-extrabold ${isSelected ? 'text-[#202124]' : 'text-slate-700'}`}>
              {opt.label}
            </span>
            <span className={`text-[10px] font-bold uppercase tracking-wider ${isSelected ? 'text-[#e6c200]' : 'text-slate-400'}`}>
              {opt.sub}
            </span>

            {/* Checkmark Badge */}
            {isSelected && (
              <div className="absolute top-2 right-2 w-2 h-2 rounded-full bg-[#e6c200] shadow-sm animate-in zoom-in" />
            )}
          </div>
        );
      })}
