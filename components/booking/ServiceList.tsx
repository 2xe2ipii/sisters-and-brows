...
export default function ServiceList() {
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 space-y-6">
      ...
      {/* SECTION 1: SESSION TYPE (Now at the top) */}
      <div className="space-y-4">
         <div className="flex items-center gap-2 text-slate-900 font-bold text-lg">
            <Layers className="w-5 h-5 text-slate-400" /> Session Type
         </div>
         <div className="flex gap-3">
            {['1ST', '2ND', 'FULL'].map((ses) => (
              <label key={ses} className="cursor-pointer flex-1">
                <input type="radio" name="session" value={ses} defaultChecked={ses === '1ST'} className="peer hidden" />
                <div className="text-center py-3 rounded-xl border-2 border-slate-100 bg-slate-50 text-slate-400 font-bold text-sm transition-all peer-checked:border-[#0f172a] peer-checked:bg-[#0f172a] peer-checked:text-white hover:border-slate-200">
                  {ses}
                </div>
              </label>
            ))}
